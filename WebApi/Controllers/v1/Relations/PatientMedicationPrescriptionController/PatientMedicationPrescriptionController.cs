...
public class PatientMedicationPrescriptionController : BaseApiController
{
    ...
    ////[Authorize(Roles = "Admin")]
    public async Task<IActionResult> Put(Guid patiendId, Guid medicationId, [FromBody] PatientMedicationPrescriptionForUpdateDto PatientMedicationPrescriptions)
    {
        var command = new UpdatePatientMedicationPrescriptionCommand(patiendId, medicationId, PatientMedicationPrescriptions, true);
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> Delete(Guid patiendId, Guid medicationId)
    {
        var notification = new DeletePatientMedicationPrescriptionCommand(patiendId, medicationId, true);
